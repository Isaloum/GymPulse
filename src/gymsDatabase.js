/**
 * Gym Locations Database
 * Real gym brands and locations across Canada
 * Comprehensive database of major fitness chains
 * 
 * Quebec brands list last updated: 2026-02-11
 * Source: Authoritative multi-location brands operating in QC
 */

/**
 * COMPREHENSIVE GYM BRANDS IN CANADA
 * All major fitness chains and independent gyms
 */
export const GYM_BRANDS = {
  // ========== QUEBEC-SPECIFIC BRANDS (Authoritative list) ==========
  
  // Major Quebec Chains
  ECONOFITNESS: 'Éconofitness',
  NAUTILUS_PLUS: 'Nautilus Plus',
  ENERGIE_CARDIO: 'Énergie Cardio',
  WORLD_GYM_QUEBEC: 'World Gym Québec',
  
  // Major National Chains (with QC presence)
  GOODLIFE_FITNESS: 'GoodLife Fitness',
  FIT4LESS: 'Fit4Less',
  PLANET_FITNESS: 'Planet Fitness',
  ANYTIME_FITNESS: 'Anytime Fitness',
  YMCA: 'YMCA',
  CRUNCH_FITNESS: 'Crunch Fitness',
  
  // Premium/Mid-Range
  ORANGETHEORY: 'Orangetheory Fitness',
  EQUINOX: 'Equinox',
  BARRY_BOOTCAMP: 'Barry\'s Bootcamp',
  RUMBLE_BOXING: 'Rumble Boxing',
  FLYWHEEL: 'Flywheel Sports',
  PELOTON: 'Peloton Studio',
  
  // CrossFit & Functional
  CROSSFIT: 'CrossFit',
  F45: 'F45',
  TOUGH_MUDDER: 'Tough Mudder',
  QUEST_FITNESS: 'Quest Fitness',
  
  // Budget/Value
  CLUB_SIX: 'Club SIX',
  IRON_WILL: 'Iron Will Fitness',
  ONE_SIZE_FITS_ALL: 'One Size Fits All',
  
  // Premium Boutique
  ELITE_GYMS: 'Elite Gyms',
  PRESTIGE_FITNESS: 'Prestige Fitness',
  STRENGTH_SPACE: 'Strength Space',
  POWER_PLATE: 'Power Plate Studios',
  
  // Yoga & Mind-Body
  LULULEMON_MIRROR: 'Lululemon Mirror',
  YOGA_ONE: 'Yoga One',
  MOKSHA_YOGA: 'Moksha Yoga',
  HOT_YOGA: 'Hot Yoga Studio',
  PURE_YOG: 'Pure Yog',
  
  // Female-Focused
  CURVES: 'Curves',
  BABES_FIT: 'Babes Fit',
  WOMEN_FIT: 'Women\'s Fitness',
  
  // Dance & Movement
  MOVE_STUDIO: 'Move Studio',
  ZUMBA: 'Zumba Live',
  POLE_DANCE: 'Pole Dance Studio',
  BARRE3: 'Barre3',
  PILATES_PLUS: 'Pilates Plus',
  
  // Specialized Training
  PERSONAL_TRAINING_PLUS: 'Personal Training +',
  BOXING_CANADA: 'Boxing Canada',
  MMA_CANADA: 'MMA Canada',
  MARTIAL_ARTS: 'Martial Arts Studio',
  
  // Budget/Student Chains
  WORKOUTS: 'Workouts.com',
  FITPLEX: 'FitPlex',
  GYMNASIUM: 'Gymnasium',
  
  // Regional Leaders
  QUEST_FITNESS_AB: 'Quest Fitness (AB)',
  STEVE_NASH_FITNESS: 'Steve Nash Fitness',
  MOVATI: 'Movati Athletic Club',
  LA_FITNESS: 'LA Fitness',
  TWENTY_FOUR_HOUR_FITNESS: '24 Hour Fitness',
  
  // Boutique/Premium
  SOLIDCORE: 'SolidCore',
  ORANGETHEORY_PREMIER: 'Orangetheory Premier',
  FLUX_STUDIO: 'Flux Studio',
  STUDIO_PLUS: 'Studio Plus',
};

export const PROVINCES = {
  QC: 'Quebec',
  ON: 'Ontario',
  BC: 'British Columbia',
  AB: 'Alberta',
  MB: 'Manitoba',
  NS: 'Nova Scotia',
  NB: 'New Brunswick',
  PE: 'Prince Edward Island',
  NL: 'Newfoundland & Labrador',
  SK: 'Saskatchewan',
};

/**
 * Quebec Gyms Database - Comprehensive multi-location brands
 * Based on authoritative sources consulted Feb 2026
 * Format: { id, name, brand, city, address, province, coordinates }
 */
export const QUEBEC_GYMS = [
  // ===== MONTREAL - Major Quebec Chains =====
  // Éconofitness (Broad QC footprint)
  {
    id: 'mtl-econofitness-1',
    name: 'Éconofitness Downtown',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Montreal',
    address: '1111 Rue Ste-Catherine Ouest',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5017, lng: -73.5673 },
  },
  {
    id: 'mtl-econofitness-2',
    name: 'Éconofitness Plateau',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Montreal',
    address: '4500 Avenue du Parc',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5244, lng: -73.5883 },
  },
  {
    id: 'mtl-econofitness-3',
    name: 'Éconofitness Griffintown',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Montreal',
    address: '1300 Rue William',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4864, lng: -73.5608 },
  },
  
  // Nautilus Plus (Quebec-wide network)
  {
    id: 'mtl-nautilus-1',
    name: 'Nautilus Plus Downtown',
    brand: GYM_BRANDS.NAUTILUS_PLUS,
    city: 'Montreal',
    address: '1425 Rue Drummond',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5041, lng: -73.5711 },
  },
  {
    id: 'mtl-nautilus-2',
    name: 'Nautilus Plus Villeray',
    brand: GYM_BRANDS.NAUTILUS_PLUS,
    city: 'Montreal',
    address: '755 Boulevard Saint-Laurent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5215, lng: -73.5783 },
  },
  
  // Énergie Cardio (Fitness club chain)
  {
    id: 'mtl-energie-1',
    name: 'Énergie Cardio Downtown',
    brand: GYM_BRANDS.ENERGIE_CARDIO,
    city: 'Montreal',
    address: '2000 Rue McGill College',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5014, lng: -73.5636 },
  },
  {
    id: 'mtl-energie-2',
    name: 'Énergie Cardio Plateau',
    brand: GYM_BRANDS.ENERGIE_CARDIO,
    city: 'Montreal',
    address: '4400 Rue Saint-Denis',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5215, lng: -73.5783 },
  },
  
  // World Gym Québec (Franchise/licensed brand model - multiple clubs)
  {
    id: 'mtl-world-gym-1',
    name: 'World Gym Québec Downtown',
    brand: GYM_BRANDS.WORLD_GYM_QUEBEC,
    city: 'Montreal',
    address: '1500 Rue Crescent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5044, lng: -73.5748 },
  },
  {
    id: 'mtl-world-gym-2',
    name: 'World Gym Québec West Island',
    brand: GYM_BRANDS.WORLD_GYM_QUEBEC,
    city: 'Montreal',
    address: '4950 Rue Jean-Talon O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5248, lng: -73.6473 },
  },

  // ===== MONTREAL - Major National Chains =====
  // GoodLife Fitness
  {
    id: 'mtl-goodlife-1',
    name: 'GoodLife Fitness Plateau',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Montreal',
    address: '4300 Rue de Bullion',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5255, lng: -73.5948 },
  },
  {
    id: 'mtl-goodlife-2',
    name: 'GoodLife Fitness NDG',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Montreal',
    address: '5960 Rue Sherbrooke O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4863, lng: -73.6265 },
  },
  
  // Fit4Less (GoodLife group budget chain)
  {
    id: 'mtl-fit4less-1',
    name: 'Fit4Less Montreal',
    brand: GYM_BRANDS.FIT4LESS,
    city: 'Montreal',
    address: '3550 Boulevard Saint-Laurent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5247, lng: -73.5876 },
  },
  
  // Planet Fitness
  {
    id: 'mtl-planet-1',
    name: 'Planet Fitness Downtown',
    brand: GYM_BRANDS.PLANET_FITNESS,
    city: 'Montreal',
    address: '1500 Rue Crescent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5044, lng: -73.5748 },
  },
  
  // Anytime Fitness (24/7 chain)
  {
    id: 'mtl-anytime-1',
    name: 'Anytime Fitness Downtown',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Montreal',
    address: '1500 Rue St-Catherine O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5017, lng: -73.5673 },
  },
  {
    id: 'mtl-anytime-2',
    name: 'Anytime Fitness Griffintown',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Montreal',
    address: '1300 Rue William',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4864, lng: -73.5608 },
  },
  
  // YMCA (Community fitness centers)
  {
    id: 'mtl-ymca-1',
    name: 'YMCA Montreal Downtown',
    brand: GYM_BRANDS.YMCA,
    city: 'Montreal',
    address: '1425 Boulevard de Maisonneuve O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5047, lng: -73.5788 },
  },
  
  // Crunch Fitness
  {
    id: 'mtl-crunch-1',
    name: 'Crunch Fitness Downtown',
    brand: GYM_BRANDS.CRUNCH_FITNESS,
    city: 'Montreal',
    address: '3030 Rue Sherbrooke Ouest',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4918, lng: -73.6084 },
  },

  // ===== MONTREAL - Boutique/Fitness Studios =====
  // Orangetheory Fitness
  {
    id: 'mtl-orangetheory-1',
    name: 'Orangetheory Fitness Old Montreal',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Montreal',
    address: '350 Rue St-Paul O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5028, lng: -73.5611 },
  },
  {
    id: 'mtl-orangetheory-2',
    name: 'Orangetheory Fitness Plateau',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Montreal',
    address: '1800 Boulevard Saint-Laurent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5215, lng: -73.5783 },
  },
  
  // F45 Training
  {
    id: 'mtl-f45-1',
    name: 'F45 Training Marais',
    brand: GYM_BRANDS.F45,
    city: 'Montreal',
    address: '4050 Rue Sainte-Catherine E',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5194, lng: -73.5668 },
  },
  
  // Rumble Boxing
  {
    id: 'mtl-rumble-1',
    name: 'Rumble Boxing Montreal',
    brand: GYM_BRANDS.RUMBLE_BOXING,
    city: 'Montreal',
    address: '1250 Rue Bishop',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5054, lng: -73.5705 },
  },
  
  // CrossFit (Affiliate network)
  {
    id: 'mtl-crossfit-1',
    name: 'CrossFit Montreal',
    brand: GYM_BRANDS.CROSSFIT,
    city: 'Montreal',
    address: '6000 Rue Saint-Urbain',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5244, lng: -73.5883 },
  },

  // ===== MONTREAL - Yoga & Wellness =====
  // Moksha Yoga
  {
    id: 'mtl-moksha-1',
    name: 'Moksha Yoga Montreal',
    brand: GYM_BRANDS.MOKSHA_YOGA,
    city: 'Montreal',
    address: '4700 Rue de Bullion',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5266, lng: -73.5941 },
  },
  
  // Pilates Plus
  {
    id: 'mtl-pilates-1',
    name: 'Pilates Plus Montreal',
    brand: GYM_BRANDS.PILATES_PLUS,
    city: 'Montreal',
    address: '3600 Rue Wellington',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4723, lng: -73.5799 },
  },
  
  // Barre3
  {
    id: 'mtl-barre-1',
    name: 'Barre3 Montreal',
    brand: GYM_BRANDS.BARRE3,
    city: 'Montreal',
    address: '1900 Avenue McGill College',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5029, lng: -73.5635 },
  },
  
  // Curves (Women's chain)
  {
    id: 'mtl-curves-1',
    name: 'Curves Montreal',
    brand: GYM_BRANDS.CURVES,
    city: 'Montreal',
    address: '2200 Rue Bleury',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5075, lng: -73.5667 },
  },
  
  // Premium
  {
    id: 'mtl-elite-1',
    name: 'Elite Gyms Westmount',
    brand: GYM_BRANDS.ELITE_GYMS,
    city: 'Montreal',
    address: '4900 Rue Sherbrooke O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4898, lng: -73.6059 },
  },

  // ===== QUEBEC CITY - Major Quebec Brands =====
  {
    id: 'qc-econofitness-1',
    name: 'Éconofitness Quebec City',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Quebec City',
    address: '1200 Rue Saint-Jean',
    province: PROVINCES.QC,
    coordinates: { lat: 46.8139, lng: -71.2080 },
  },
  {
    id: 'qc-nautilus-1',
    name: 'Nautilus Plus Quebec City',
    brand: GYM_BRANDS.NAUTILUS_PLUS,
    city: 'Quebec City',
    address: '1000 Rue d\'Aiguillon',
    province: PROVINCES.QC,
    coordinates: { lat: 46.8156, lng: -71.2022 },
  },
  {
    id: 'qc-energie-1',
    name: 'Énergie Cardio Sainte-Foy',
    brand: GYM_BRANDS.ENERGIE_CARDIO,
    city: 'Quebec City',
    address: '2700 Boulevard Laurier',
    province: PROVINCES.QC,
    coordinates: { lat: 46.7909, lng: -71.2265 },
  },
  {
    id: 'qc-world-gym-1',
    name: 'World Gym Québec Sainte-Foy',
    brand: GYM_BRANDS.WORLD_GYM_QUEBEC,
    city: 'Quebec City',
    address: '2750 Boulevard Laurier',
    province: PROVINCES.QC,
    coordinates: { lat: 46.7915, lng: -71.2275 },
  },

  // National chains in Quebec City
  {
    id: 'qc-anytime-1',
    name: 'Anytime Fitness Vieux-Quebec',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Quebec City',
    address: '580 Rue de la Reine',
    province: PROVINCES.QC,
    coordinates: { lat: 46.8139, lng: -71.2080 },
  },
  {
    id: 'qc-goodlife-1',
    name: 'GoodLife Fitness Sainte-Foy',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Quebec City',
    address: '2600 Boulevard Laurier',
    province: PROVINCES.QC,
    coordinates: { lat: 46.7909, lng: -71.2265 },
  },
  {
    id: 'qc-ymca-1',
    name: 'YMCA Quebec City',
    brand: GYM_BRANDS.YMCA,
    city: 'Quebec City',
    address: '855 Avenue Holland',
    province: PROVINCES.QC,
    coordinates: { lat: 46.8172, lng: -71.1968 },
  },
  {
    id: 'qc-orangetheory-1',
    name: 'Orangetheory Fitness Quebec',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Quebec City',
    address: '2600 Rue de la Faïencerie',
    province: PROVINCES.QC,
    coordinates: { lat: 46.8220, lng: -71.1544 },
  },

  // ===== GATINEAU (National Capital Region) - Western QC presence =====
  {
    id: 'gatineau-goodlife-1',
    name: 'GoodLife Fitness Hull',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Gatineau',
    address: '158 Rue de la Cathédrale',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4395, lng: -75.7391 },
  },
  {
    id: 'gatineau-anytime-1',
    name: 'Anytime Fitness Gatineau',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Gatineau',
    address: '415 Boulevard de la Cité',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4423, lng: -75.7369 },
  },
  {
    id: 'gatineau-fit4less-1',
    name: 'Fit4Less Gatineau',
    brand: GYM_BRANDS.FIT4LESS,
    city: 'Gatineau',
    address: '250 Boulevard Saint-Joseph',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4497, lng: -75.7470 },
  },
  {
    id: 'gatineau-ymca-1',
    name: 'YMCA Hull',
    brand: GYM_BRANDS.YMCA,
    city: 'Gatineau',
    address: '167 Rue de la Cathédrale',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4395, lng: -75.7385 },
  },
  {
    id: 'gatineau-f45-1',
    name: 'F45 Training Gatineau',
    brand: GYM_BRANDS.F45,
    city: 'Gatineau',
    address: '220 rue Principale',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4316, lng: -75.7446 },
  },

  // ===== SHERBROOKE =====
  {
    id: 'sherbrooke-econofitness-1',
    name: 'Éconofitness Sherbrooke',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Sherbrooke',
    address: '1800 Rue King Ouest',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4015, lng: -71.8947 },
  },
  {
    id: 'sherbrooke-world-gym-1',
    name: 'World Gym Québec Sherbrooke',
    brand: GYM_BRANDS.WORLD_GYM_QUEBEC,
    city: 'Sherbrooke',
    address: '1900 Rue King Ouest',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4015, lng: -71.8920 },
  },
  {
    id: 'sherbrooke-club-six-1',
    name: 'Club SIX Sherbrooke',
    brand: GYM_BRANDS.CLUB_SIX,
    city: 'Sherbrooke',
    address: '365 Boulevard Industriel',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4088, lng: -71.8750 },
  },
  {
    id: 'sherbrooke-ymca-1',
    name: 'YMCA Sherbrooke',
    brand: GYM_BRANDS.YMCA,
    city: 'Sherbrooke',
    address: '1100 Rue de Montréal',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4101, lng: -71.8863 },
  },
  {
    id: 'sherbrooke-anytime-1',
    name: 'Anytime Fitness Sherbrooke',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Sherbrooke',
    address: '3150 Rue King Ouest',
    province: PROVINCES.QC,
    coordinates: { lat: 45.3977, lng: -71.9443 },
  },
  {
    id: 'sherbrooke-energie-1',
    name: 'Énergie Cardio Sherbrooke',
    brand: GYM_BRANDS.ENERGIE_CARDIO,
    city: 'Sherbrooke',
    address: '50 Rue Wellington Nord',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4041, lng: -71.9051 },
  },

  // ===== LAVAL =====
  {
    id: 'laval-nautilus-1',
    name: 'Nautilus Plus Chomedey',
    brand: GYM_BRANDS.NAUTILUS_PLUS,
    city: 'Laval',
    address: '3595 Boulevard Chomedey',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5731, lng: -73.7391 },
  },
  {
    id: 'laval-energie-1',
    name: 'Énergie Cardio Vimont',
    brand: GYM_BRANDS.ENERGIE_CARDIO,
    city: 'Laval',
    address: '2800 Boulevard Vimont',
    province: PROVINCES.QC,
    coordinates: { lat: 45.6045, lng: -73.7208 },
  },
  {
    id: 'laval-econofitness-1',
    name: 'Éconofitness Laval',
    brand: GYM_BRANDS.ECONOFITNESS,
    city: 'Laval',
    address: '3700 Boulevard Chomedey',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5745, lng: -73.7406 },
  },
  {
    id: 'laval-goodlife-1',
    name: 'GoodLife Fitness Laval',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Laval',
    address: '2180 Boulevard des Laurentides',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5605, lng: -73.7454 },
  },
  {
    id: 'laval-anytime-1',
    name: 'Anytime Fitness Vimont',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Laval',
    address: '440 Boulevard de la Concorde O',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5733, lng: -73.7547 },
  },

  // ===== LONGUEUIL =====
  {
    id: 'longueuil-goodlife-1',
    name: 'GoodLife Fitness Longueuil',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Longueuil',
    address: '195 Rue Saint-Laurent',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5166, lng: -73.5033 },
  },
  {
    id: 'longueuil-crunch-1',
    name: 'Crunch Fitness Longueuil',
    brand: GYM_BRANDS.CRUNCH_FITNESS,
    city: 'Longueuil',
    address: '1900 Boulevard Taschereau',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5120, lng: -73.5144 },
  },
  {
    id: 'longueuil-anytime-1',
    name: 'Anytime Fitness Longueuil',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Longueuil',
    address: '1600 Boulevard Taschereau',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5145, lng: -73.5118 },
  },
  {
    id: 'longueuil-f45-1',
    name: 'F45 Training Longueuil',
    brand: GYM_BRANDS.F45,
    city: 'Longueuil',
    address: '2000 Boulevard Taschereau',
    province: PROVINCES.QC,
    coordinates: { lat: 45.5090, lng: -73.5170 },
  },

  // ===== ADDITIONAL MAJOR CITIES =====
  {
    id: 'brossard-planet-1',
    name: 'Planet Fitness Brossard',
    brand: GYM_BRANDS.PLANET_FITNESS,
    city: 'Brossard',
    address: '8000 Boulevard Leduc',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4586, lng: -73.4863 },
  },
  {
    id: 'brossard-orangetheory-1',
    name: 'Orangetheory Fitness Brossard',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Brossard',
    address: '7900 Boulevard Taschereau',
    province: PROVINCES.QC,
    coordinates: { lat: 45.4598, lng: -73.4797 },
  },
];

/**
 * Ontario Gyms Database (Major Cities)
 */
export const ONTARIO_GYMS = [
  // Toronto
  {
    id: 'tor-goodlife-1',
    name: 'GoodLife Fitness Downtown Toronto',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Toronto',
    address: '181 Bay Street',
    province: PROVINCES.ON,
    coordinates: { lat: 43.6426, lng: -79.3871 },
  },
  {
    id: 'tor-anytime-1',
    name: 'Anytime Fitness Toronto',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Toronto',
    address: '20 Queen Street W',
    province: PROVINCES.ON,
    coordinates: { lat: 43.6520, lng: -79.3795 },
  },
  {
    id: 'tor-equinox-1',
    name: 'Equinox Toronto',
    brand: GYM_BRANDS.EQUINOX,
    city: 'Toronto',
    address: '250 Bloor Street W',
    province: PROVINCES.ON,
    coordinates: { lat: 43.6690, lng: -79.3957 },
  },
  {
    id: 'tor-orangetheory-1',
    name: 'Orangetheory Fitness Toronto',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Toronto',
    address: '317 Dundas Street W',
    province: PROVINCES.ON,
    coordinates: { lat: 43.6606, lng: -79.3903 },
  },

  // Ottawa
  {
    id: 'ottawa-goodlife-1',
    name: 'GoodLife Fitness Ottawa',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Ottawa',
    address: '250 Tremblay Road',
    province: PROVINCES.ON,
    coordinates: { lat: 45.4215, lng: -75.6972 },
  },
  {
    id: 'ottawa-anytime-1',
    name: 'Anytime Fitness Ottawa',
    brand: GYM_BRANDS.ANYTIME_FITNESS,
    city: 'Ottawa',
    address: '1 Sparks Street',
    province: PROVINCES.ON,
    coordinates: { lat: 45.4215, lng: -75.6989 },
  },
];

/**
 * British Columbia Gyms (Major Cities)
 */
export const BC_GYMS = [
  // Vancouver
  {
    id: 'van-goodlife-1',
    name: 'GoodLife Fitness Vancouver',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Vancouver',
    address: '1055 Homer Street',
    province: PROVINCES.BC,
    coordinates: { lat: 49.2824, lng: -123.1207 },
  },
  {
    id: 'van-steve-nash-1',
    name: 'Steve Nash Fitness Vancouver',
    brand: GYM_BRANDS.STEVE_NASH_FITNESS,
    city: 'Vancouver',
    address: '1835 Barclay Street',
    province: PROVINCES.BC,
    coordinates: { lat: 49.2829, lng: -123.1636 },
  },
  {
    id: 'van-orangetheory-1',
    name: 'Orangetheory Fitness Vancouver',
    brand: GYM_BRANDS.ORANGETHEORY,
    city: 'Vancouver',
    address: '743 Seymour Street',
    province: PROVINCES.BC,
    coordinates: { lat: 49.2823, lng: -123.1141 },
  },
];

/**
 * Alberta Gyms (Major Cities)
 */
export const ALBERTA_GYMS = [
  // Calgary
  {
    id: 'calgary-goodlife-1',
    name: 'GoodLife Fitness Calgary',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Calgary',
    address: '250 Centre Street S',
    province: PROVINCES.AB,
    coordinates: { lat: 51.0447, lng: -104.6368 },
  },
  {
    id: 'calgary-quest-1',
    name: 'Quest Fitness Calgary',
    brand: GYM_BRANDS.QUEST_FITNESS_AB,
    city: 'Calgary',
    address: '11 Aspen Landing Boulevard',
    province: PROVINCES.AB,
    coordinates: { lat: 51.0329, lng: -114.2082 },
  },

  // Edmonton
  {
    id: 'edmonton-goodlife-1',
    name: 'GoodLife Fitness Edmonton',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Edmonton',
    address: '10220 103 Avenue NW',
    province: PROVINCES.AB,
    coordinates: { lat: 53.5444, lng: -113.5119 },
  },
  {
    id: 'edmonton-movati-1',
    name: 'Movati Athletic Club Edmonton',
    brand: GYM_BRANDS.MOVATI,
    city: 'Edmonton',
    address: '124 Street & 102 Avenue',
    province: PROVINCES.AB,
    coordinates: { lat: 53.5362, lng: -113.5043 },
  },
];

/**
 * Manitoba & Other Provinces
 */
export const MANITOBA_GYMS = [
  {
    id: 'wpg-goodlife-1',
    name: 'GoodLife Fitness Winnipeg',
    brand: GYM_BRANDS.GOODLIFE_FITNESS,
    city: 'Winnipeg',
    address: '201 Portage Avenue',
    province: PROVINCES.MB,
    coordinates: { lat: 49.8951, lng: -97.1384 },
  },
];

/**
 * Get all gyms for a specific province
 */
export const getGymsByProvince = (province) => {
  const provinceKey = Object.keys(PROVINCES).find(
    (key) => PROVINCES[key] === province
  );

  switch (provinceKey) {
    case 'QC':
      return QUEBEC_GYMS;
    case 'ON':
      return ONTARIO_GYMS;
    case 'BC':
      return BC_GYMS;
    case 'AB':
      return ALBERTA_GYMS;
    case 'MB':
      return MANITOBA_GYMS;
    default:
      return QUEBEC_GYMS; // Default to Quebec
  }
};

/**
 * Get all cities in a province
 */
export const getCitiesByProvince = (province) => {
  const gyms = getGymsByProvince(province);
  const cities = [...new Set(gyms.map((gym) => gym.city))];
  return cities.sort();
};

/**
 * Get gyms filtered by province and city
 */
export const getGymsByProvinceAndCity = (province, city = null) => {
  let gyms = getGymsByProvince(province);
  if (city) {
    gyms = gyms.filter((gym) => gym.city === city);
  }
  return gyms;
};

/**
 * Get gym by ID
 */
export const getGymById = (gymId) => {
  const allGyms = [
    ...QUEBEC_GYMS,
    ...ONTARIO_GYMS,
    ...BC_GYMS,
    ...ALBERTA_GYMS,
    ...MANITOBA_GYMS,
  ];
  return allGyms.find((gym) => gym.id === gymId);
};

/**
 * Search gyms by name or brand
 */
export const searchGyms = (query, province = null) => {
  const gyms = province ? getGymsByProvince(province) : [
    ...QUEBEC_GYMS,
    ...ONTARIO_GYMS,
    ...BC_GYMS,
    ...ALBERTA_GYMS,
    ...MANITOBA_GYMS,
  ];
  const lowerQuery = query.toLowerCase();
  return gyms.filter(
    (gym) =>
      gym.name.toLowerCase().includes(lowerQuery) ||
      gym.brand.toLowerCase().includes(lowerQuery) ||
      gym.city.toLowerCase().includes(lowerQuery)
  );
};

/**
 * Get all unique gym brands (optionally filtered by province)
 */
export const getAllBrands = (province = null) => {
  const gyms = province ? getGymsByProvince(province) : [
    ...QUEBEC_GYMS,
    ...ONTARIO_GYMS,
    ...BC_GYMS,
    ...ALBERTA_GYMS,
    ...MANITOBA_GYMS,
  ];
  const brands = [...new Set(gyms.map((gym) => gym.brand))];
  return brands.sort();
};
